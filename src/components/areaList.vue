<template>
  <div>
    <el-row class="moveBlock">
      <el-col :span="24">
        <el-row class="topSearch">
          <el-col :span="5">
            <el-button class="addUser" @click="dialogVisible = true;">添加区域</el-button>
          </el-col>
    ...
    <el-dialog title="添加区域" :visible.sync="dialogVisible" size="tiny" @close="close">
      <el-row>
        <el-col :span="6">
          区域名称
        </el-col>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addAreaBtn">确 定</el-button>
      </div>
    </el-dialog>
...
<script>
  export default {
    ...
    methods: {
      ...
      addAreaBtn() {
        if (!this.form.name) return this.$message('请输入区域名称')

        if (this.edit) {
          this.$axios.put({
            url: `/api/areas?name=${this.form.name}&id=${this.form.id}`,
            token: this.token
          }).then(res => {
            console.log(res.data);
            if (res.data.success) {
          ...
          this.$axios.post({
            url: `/api/areas?name=${this.form.name}`,
            token: this.token
          }).then(res => {
            console.log(res.data);
            if (res.data.success) {
